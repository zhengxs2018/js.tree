...
function map(data, callback, childrenKey = CHILDREN_KEY) {
    function iter(data, parents) {
        return data.map((node, index) => {
            ...
            const children = iter(lodash.defaultTo(source[childrenKey], []), parents.concat(node));
...
function each(data, callback, childrenKey = CHILDREN_KEY) {
    function iter(data, parents) {
        ...
        data.forEach((node, index) => {
            ...
            iter(lodash.defaultTo(node[childrenKey], []), parents.concat(node));
        });
        return items;
    }
    return iter(data, []);
...
function filter(data, callback, childrenKey = CHILDREN_KEY) {
    function iter(data, parents) {
        ...
        data.forEach((node, index) => {
            ...
            const children = iter(lodash.defaultTo(node[childrenKey], []), parents.concat(node));
            if (children.length > 0) {
...
function exclude(data, callback, childrenKey = CHILDREN_KEY) {
    function iter(data, parents) {
        ...
        data.forEach((node, index) => {
            ...
            // 判读是否存在子级
            const children = lodash.defaultTo(node[childrenKey], []);
            if (children.length === 0) {
                items.push(node);
                return;
            }
...
function isNotNil(value) {
    return lodash.isNil(value) === false;
}
/**
 * 断言，模拟 node api
 *
...
function exporter(nodes, root) {
    ...
    return nodes[lodash.defaultTo(root, ROOT_ID)] || [];
}
...
function parse(data, options = {}) {
    const idKey = lodash.defaultTo(options.idKey, ID_KEY);
    const parentKey = lodash.defaultTo(options.parentKey, PARENT_ID_KEY);
    const childrenKey = lodash.defaultTo(options.childrenKey, CHILDREN_KEY);
    const transform = lodash.defaultTo(options.transform, (x) => x);
    const insert = lodash.defaultTo(options.insert, (siblings, node) => siblings.push(node));
    const nodes = {};
    const childNodes = {};
    data.forEach((row, i) => {
        // 获取节点ID
